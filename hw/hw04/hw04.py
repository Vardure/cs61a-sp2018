...
def taxicab(a, b):
    ...
    street2 = street(b)
    avenue2 = avenue(b)
    return abs(street1 - street2) + abs(avenue1 - avenue2)
...
def squares(s):
    ...
    squares_roots = []
    for integer in s:
        if round(integer ** 0.5) ** 2 == integer:
            #Can only concatenate list to list
            squares_roots = squares_roots + [round(integer ** 0.5)]
    return squares_roots
